...
function loadInfo() {
  ...
  Object.keys(player.attributes).forEach(attr => {
    ...
    if (player.attributes[attr].level !== level) {
      ...
      updated = true;
      player.activityLog.push(`${attr} is now Level ${level}`);
    }
  });
...
function loadActions() {
  ...
  if (player.lastTrainingDate !== today) {
    ...
    player.activityLog.push("New day: Training and task limits reset.");
    savePlayer();
  }
  ...
  if (player.training) {
    const minutes = Math.max(0, Math.ceil((player.training.end - Date.now()) / 60000));
  ...
  if (player.task) {
    const minutes = Math.max(0, Math.ceil((player.task.end - Date.now()) / 60000));
    html += `<p>Task '${player.task.name}' in progress. Time left: ${minutes} min</p>`;
  } else if (!player.completedTaskToday) {
    html += `<select id="taskSelect">` + getTodayTasks().map(t => `<option value="${t.name}">${t.name}</option>`).join('') + `</select>`;
    html += `<button onclick="startTask()">Start Task (25 energy)</button>`;
  } else {
    html += "<p>Task already completed today.</p>";
  }

  html += `<h3>Activity Log</h3><ul>`;
  player.activityLog.slice(-5).reverse().forEach(entry => {
    html += `<li>${entry}</li>`;
  });
  html += `</ul>`;
